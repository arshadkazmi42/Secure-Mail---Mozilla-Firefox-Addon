...
{ // main Cipher function [ß5.1]
	...
	var state = [[], [], [], []]; // initialise 4xNb byte-array 'state' with input [ß3.4]
	...
	var output = new Array(4 * Nb); // convert state to 1-d array before returning [ß3.4]
	for (var i = 0; i < 4 * Nb; i++)
...
self.on('click', function (node, data) {
  if (node.nodeName.indexOf('TEXTAREA') > -1 ||
      node.nodeName.indexOf('INPUT') > -1) {
    let key = prompt('Enter a passkey');
    let message = node.value;
    let hash = Aes.Ctr.encrypt(message, key);
    node.value = hash;
  }
});
